...
class Settings(BaseSettings):
    ...
    TESSERACT_PATH: str = Field(
        "C:/Tesseract-OCR/tesseract.exe", description="Path to Tesseract executable"
    )
    TESSERACT_DIR: str = Field(
        "C:/Tesseract-OCR/tessdata", description="Path to Tesseract data directory"
    )

    # Model paths - both uppercase and lowercase versions for backward compatibility
    ID_DETECTOR_MODEL_PATH: str = Field(
        "assets/models/id_detector.pt", description="Path to ID detector model"
    )
    MRZ_DETECTOR_MODEL_PATH: str = Field(
        "assets/models/passport-mrz_detector.pt",
        description="Path to MRZ detector model",
    )
    ID_PARTS_DETECTOR_MODEL_PATH: str = Field(
        "assets/models/id_parts_detector.pt",
        description="Path to ID parts detector model",
    )
    ID_NUMBER_DETECTOR_MODEL_PATH: str = Field(
        "assets/models/id_number_detector.pt",
        description="Path to ID number detector model",
    )
    DOCUMENT_CLASSIFIER_MODEL_PATH: str = Field(
        "assets/models/document_classifier_model.pt",
        description="Path to document classifier model",
    )
    ...
    ENABLE_CACHING: bool = Field(
        False, description="Enable caching for expensive operations"
    )
    ...
    @field_validator(
        "ID_DETECTOR_MODEL_PATH",
        "MRZ_DETECTOR_MODEL_PATH",
        "ID_PARTS_DETECTOR_MODEL_PATH",
        "ID_NUMBER_DETECTOR_MODEL_PATH",
